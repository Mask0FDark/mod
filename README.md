...
Mailpit/MailHog сохраняют письма локально и не доставляют их на реальную почту. Такие серверы допускаются только для тестовых адресов `@example.test`; внешняя регистрация получает `mail_not_configured`. Для другого тестового SMTP явно задайте `SMTP_TEST_ONLY=true`.
...
Браузерные регрессии доставки (Python Playwright и установленный Microsoft Edge):

```powershell
python tests/delivery_browser.py .
```

Тесты используют изолированные страницы и подставные API/crypto-ответы с задержками, без доступа к реальным аккаунтам.
Они проверяют конкуренцию истории и WebSocket, порядок и уникальность сообщений, смену чата во время расшифровки,
повторную отправку, черновики, read-state, комментарии и восстановление соединения.
Проверка живого dev дополнительно необходима для базы, настоящего шифрования и двух клиентов.

Новые отправки передают UUID `clientMessageId`. Повтор того же запроса возвращает существующее сообщение;
другой payload с тем же UUID отклоняется с HTTP 409. Старые клиенты без UUID остаются совместимыми.
Миграция добавляет nullable-колонки и уникальный индекс и не удаляет историю переписки.
