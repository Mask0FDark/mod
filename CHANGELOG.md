...
- Кнопки действий доступны при наведении на строку сообщения.
- Mailpit/MailHog не считаются настоящей доставкой: внешние регистрации получают понятную ошибку конфигурации.

- Прекращён цикл `read → загрузка истории → read`; сервер рассылает только продвижение курсора.
- Параллельная загрузка истории и WebSocket больше не создают повторные пузырьки; порядок определяется ID.
- Запоздавшие ответы и расшифровка не изменяют другой открытый чат или тред.
- Защита от двойного нажатия отправки и идемпотентный `clientMessageId` для повторов после потери ответа.
- Набранный во время отправки текст сохраняется; чтение фоновой вкладки не отправляет подтверждения.
- При восстановлении WebSocket повторно загружается история; отметки прочтения обновляются без перерисовки истории.
- Добавлены браузерные регрессии доставки с управляемыми задержками.
